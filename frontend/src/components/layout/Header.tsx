...
export default function Header() {
  ...
  // Wallet-dependent values
  const walletConnected = !!account;
  const walletConnecting = isConnecting;
          ...
          {mounted ? (
            ...
            >
              ...
              <span className="relative z-10">
                {walletConnecting ? 'Connecting...' : walletConnected ? fmt(account!) : 'Connect Wallet'}
              </span>
              {walletConnected && <ChevronDown size={12} className={`relative z-10 opacity-50 transition-transform ${showDropdown ? 'rotate-180' : ''}`} />}
            </button>
          ) : (
            /* Server-side / Initial mount placeholder to prevent mismatch */
            <div className="flex items-center gap-2.5 px-5 py-2.5 rounded-xl text-[12px] font-bold text-white/50 border border-white/[0.08] bg-white/[0.02]">
              <Wallet size={14} className="opacity-50" />
              <span>Connect Wallet</span>
            </div>
          )}

          {mounted && walletConnected && showDropdown && (
