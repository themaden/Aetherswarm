...
exports.ROUTER_AS_RECIPIENT = exports.SENDER_AS_RECIPIENT = exports.MAX_UINT160 = exports.MAX_UINT256 = exports.ZERO_ADDRESS = exports.E_ETH_ADDRESS = exports.ETH_ADDRESS = exports.CONTRACT_BALANCE = exports.WETH_ADDRESS = exports.UNIVERSAL_ROUTER_CREATION_BLOCK = exports.SWAP_PROXY_ADDRESS = exports.UNIVERSAL_ROUTER_ADDRESS = exports.CHAIN_CONFIGS = exports.UniversalRouterVersion = void 0;
...
})(UniversalRouterVersion = exports.UniversalRouterVersion || (exports.UniversalRouterVersion = {}));
...
const UNIVERSAL_ROUTER_ADDRESS = (version, chainId) => {
    if (!(chainId in exports.CHAIN_CONFIGS))
        throw new Error(`Universal Router not deployed on chain ${chainId}`);
    const config = exports.CHAIN_CONFIGS[chainId].routerConfigs[version];
    if (!config)
        throw new Error(`Universal Router version ${version} not deployed on chain ${chainId}`);
    return config.address;
};
exports.UNIVERSAL_ROUTER_ADDRESS = UNIVERSAL_ROUTER_ADDRESS;
const SWAP_PROXY_ADDRESS = (chainId) => {
    if (!(chainId in exports.CHAIN_CONFIGS))
        throw new Error(`SwapProxy not deployed on chain ${chainId}`);
    const proxy = exports.CHAIN_CONFIGS[chainId].swapProxy;
    if (!proxy)
        throw new Error(`SwapProxy not configured for chain ${chainId}`);
    return proxy;
};
exports.SWAP_PROXY_ADDRESS = SWAP_PROXY_ADDRESS;
const UNIVERSAL_ROUTER_CREATION_BLOCK = (version, chainId) => {
    if (!(chainId in exports.CHAIN_CONFIGS))
        throw new Error(`Universal Router not deployed on chain ${chainId}`);
    const config = exports.CHAIN_CONFIGS[chainId].routerConfigs[version];
    if (!config)
...
const WETH_ADDRESS = (chainId) => {
    if (!(chainId in exports.CHAIN_CONFIGS))
        throw new Error(`Universal Router not deployed on chain ${chainId}`);
    if (exports.CHAIN_CONFIGS[chainId].weth == WETH_NOT_SUPPORTED_ON_CHAIN)
        throw new Error(`Chain ${chainId} does not have WETH`);
    return exports.CHAIN_CONFIGS[chainId].weth;
};
exports.WETH_ADDRESS = WETH_ADDRESS;
exports.CONTRACT_BALANCE = ethers_1.BigNumber.from(2).pow(255);
...
exports.MAX_UINT160 = ethers_1.BigNumber.from(2).pow(160).sub(1);
